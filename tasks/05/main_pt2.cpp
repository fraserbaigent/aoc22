...
void moveCrates(FileReader & reader, CrateStack & crate_stack) {
    ...
    std::vector<char> intermediates(number_to_move, ' ');
    for (int i = number_to_move - 1; i>=0; i--) {
	intermediates[i] = crate_stack[from-1].top();
	crate_stack[from-1].pop();
    }

    for (auto c : intermediates) {
	crate_stack[to-1].emplace(c);
    }
}
